...
class SALight(EnvironmentLight):
    def shade(self, gb_pos, gb_normal, kd, ks, view_pos, specular=True):
        wo = util.safe_normalize(view_pos - gb_pos)
        wi = util.safe_normalize(util.reflect(wo, gb_normal))
        epsilon = 1e-6  # 避免除零错误

        H = util.safe_normalize(wi + wo)
        ...
        if specular:
            ...
            F_0 = metallic * 0.9 + (1 - metallic) * 0.04  # 根据金属度调整 F_0
            F = F_0 + (1.0 - F_0) * (1.0 - torch.dot(H, wo)) ** 5  # Schlick近似

            G = (2.0 * torch.dot(gb_normal, wo)) / (torch.dot(wo, H) + epsilon) * (2.0 * torch.dot(gb_normal, wi)) / (
                        torch.dot(wi, H) + epsilon)
